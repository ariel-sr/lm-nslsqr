// nslsqr_mf.cu
// Implementation file for the nslsqr_mf variation.
...
////////////////////////////////////////////////////////////////
__global__
void kernel_get_row2(double *dst, double *src, int ld, int rows, int row, double scal)
{
	// Get position
...
)
{
	...
	cuda_check_error(cudaMemcpy(dev_dst + M, dev_v, sizeof(double)*N, cudaMemcpyDeviceToDevice), "augmented_dot2, Copy lower part of dev_dst\n");
	// Scaling the lower part of dev_dst -> s*v
	cublas_check_error(cublasDscal(*handle, N, &s, dev_dst + M, 1), "augmented_dot2, Scaling lower part of dev_dst\n");
}
...
)
{
	...
	jac_transpose(M, N, func, dev_xi, dev_w, dev_dst, eps, qm->handle);
	// Now we need to compute dev_dst := dev_dst + s*w[M+1:M+N]
	cublas_check_error(cublasDaxpy(*qm->handle, N, &s, dev_w+M, 1, dev_dst, 1), "augmented_tdot2, axpy for scaling the identity matrix\n");
}
...
)
{
	...
	// residual list for slope computation and it iteration
	...
	for (int i = 0; i < restart; ++i) {
		...
		augmented_tdot2(qm->M, qm->N, func, qm, dev_xi, dev_hat_r0, sdamp, 1e-6, dev_r0);
		...
		for (int k = 0; k < maxit; ++k) {
			...
			augmented_dot2(qm->M, qm->N, func, dev_xi, dev_V + N*k, sdamp, dev_auxM, 1e-6, qm->handle);
			...
			if (alpha > 1e-12) {
				...
				if (k < maxit-1) {
					...
					// Next V vector
					augmented_tdot2(qm->M, qm->N, func, qm, dev_xi, dev_U + M*(k+1), sdamp, 1e-6, dev_auxN2);
					...
					while (continue_V_vector) {
						...
						if (alpha > 1e-12) {
							...
							if (debug)
								fprintf(stderr, "A zero value in generation of V: adding a small perturbation\n");
			...
			kernel_get_row2<<<((k+1)+255)/256, 256>>>(dev_auxQ, dev_Q, maxit+1, k+1, 0, norm_hat_r0);
			...
			augmented_dot2(qm->M, qm->N, func, dev_xi, dev_x, sdamp, dev_auxM, 1e-6, qm->handle);
			alpha = -1;
